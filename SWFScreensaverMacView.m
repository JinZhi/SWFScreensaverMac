...
@synthesize showInfoPic;
@synthesize showInfoText;
@synthesize showConfig;
...
@synthesize interactiveMode;
@synthesize pMainBundle;
@synthesize pBundlePath;

@synthesize infoDict;
@synthesize pathToLogFile;
...
- (id)initWithFrame:(NSRect)frame isPreview:(BOOL)isPreview {
    ...
    if (self) {
		...
		if (debugMode) {
            ...
            [contentsOfFile autorelease]; 
			
			// if file doesn't exist, create it
			if (myError != nil) {
				contentsOfFile = @"";
				[contentsOfFile writeToFile:pathToLogFile atomically:YES encoding:NSUTF8StringEncoding error:NULL];
			}
		}
...
static Boolean checkOnline(const char *serverName) {
    ...
    SCNetworkReachabilityRef reachability = SCNetworkReachabilityCreateWithName(NULL,serverName);
